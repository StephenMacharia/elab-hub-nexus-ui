...
const defaultMetrics: WellnessMetric[] = [
  {
    id: 'steps',
    name: 'Daily Steps',
    value: 8432,
...
// Generate random values between 60–100% of each target
const generateRandomMetrics = (): WellnessMetric[] => {
  return defaultMetrics.map(metric => {
    const value = Math.floor(
      Math.random() * (metric.target * 1.1 - metric.target * 0.6) + metric.target * 0.6
    );
    return { ...metric, value };
  });
};
...
const WellnessTracker = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('wellnessMetrics');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('wellnessMetrics', JSON.stringify(metrics));
  }, [metrics]);

  const resetMetrics = () => {
    const randomMetrics = generateRandomMetrics();
    setMetrics(randomMetrics);
    localStorage.setItem('wellnessMetrics', JSON.stringify(randomMetrics));
  };
